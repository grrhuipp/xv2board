...
foreach (['fetch' => 'GET', 'mark' => 'POST'] as $action => $method) {
...
try {
    ...
    $log($a->id, '2026-09-12 11:59:59', '192.0.2.90');
    $log($a->id, '2026-09-12 12:00:00', '192.0.2.1');
    ...
    checkAnalysis($result['total'] === 2 && (int) $result['summary']->requests === 11, '24h range excludes future, old, deleted and inactive users');
    $row = collect($result['data'])->firstWhere('user_id', $a->id);
    checkAnalysis([$row->count_2m, $row->count_5m, $row->count_1h, $row->count_24h] === [5, 6, 8, 9], 'rolling request windows include exact boundaries');
    checkAnalysis([$row->ips_10m, $row->ips_24h] === [3, 5], 'distinct IP windows deduplicate');
    checkAnalysis($row->latest->user_agent === '<img src=x onerror=alert(1)> 100% client', 'latest event uses timestamp then id');
    checkAnalysis($row->average_interval === 10800, 'mean interval uses span divided by gaps');
    checkAnalysis(collect($result['data'])->firstWhere('user_id', $b->id)->ips_24h === 0, 'missing IPs excluded');
