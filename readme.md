...
「系统配置 → APP」管理 App 客户端接口前缀（默认 `/api/v1/app`）、更新 JSON 和加密密钥。SmartRoute 接口跟随同一前缀；更改后须同步客户端地址并重启常驻服务。旧客户端如需继续使用原路径，可在后台配置对应前缀。管理员配置接口会回传 AES 密钥与 IV 以便表单回显；后台仅在主动点击「显示」后明文展示，请保护管理员会话和浏览器环境，不要缓存配置响应。
