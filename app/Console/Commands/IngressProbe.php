 ...
 *
 * 锁模型（修复「锁自杀」缺陷）：
 *   - Laravel 锁 get() 是 NX 获取语义，对本进程已持有的同一把锁再次 get() 必返回
 *     false。原循环体内的二次 get() 会让 daemon 第一轮（含 --once）在探活前就误判
 *     「丢锁」退出。现改为：handle() 开头获取一次锁（TTL=0，整个生命周期持有），
 *     循环体内不再触碰锁，仅刷新独立的心跳 key 表达存活；进程退出时 release()。
 *
 * 每轮 tick：
...
class IngressProbe extends Command
{
    ...
    public function handle(): int
    {
        ...
        // 单实例锁（尽力而为）：Redis 可用时用锁防多实例重入；Redis 不可用时
        // 不阻止运行——否则 Redis 抖动期 supervisor 反复重启却永远拿不到锁，
        // 等于探活彻底停摆。健康数据落 MySQL，不依赖 Redis，宁可短暂多实例并发
        // （upsert 幂等、写库无害）也不能整体停摆。
        $lock = $this->tryAcquireLock();
        if ($lock === false) {
            // 拿锁失败：可能是孤儿锁（持锁进程被强杀，未释放）。检查心跳是否已过期：
            // 心跳过期说明「持锁者实际已死」，强制释放并重试一次，实现重启后立即自愈，
            // 无需干等锁 TTL 到期。心跳新鲜则说明确有活实例在跑，正常退出。
            if ($this->isHeartbeatStale()) {
                Log::warning('[IngressProbe] stale lock detected (heartbeat expired), force releasing');
                $lock = $this->forceReacquireLock();
            }
            if (!$lock) {
    ...
    /** 心跳是否已过期（持锁者疑似已死）。无心跳或心跳早于 lock_ttl 视为过期。 */
    private function isHeartbeatStale(): bool
    {
        try {
            $hb = Cache::get(self::HEARTBEAT_KEY);
            if (!is_array($hb) || empty($hb['at'])) {
                return true; // 无心跳 → 视为无活实例
            }
            return (time() - (int)$hb['at']) > $this->lockTtl();
        } catch (\Throwable $e) {
            return false; // Redis 读不到时保守，不强制抢锁
        }
    }

    /** 强制释放孤儿锁并重新获取。 */
    private function forceReacquireLock()
    {
        try {
            $this->lockStore()->lock(self::LOCK_KEY)->forceRelease();
            $lock = $this->lockStore()->lock(self::LOCK_KEY, $this->lockTtl());
            return $lock->get() ? $lock : false;
        } catch (\Throwable $e) {
            Log::warning('[IngressProbe] force reacquire failed', ['msg' => $e->getMessage()]);
            return false;
        }
    }
     ...
     *
     * @return mixed  锁对象（拿到）/ null（Redis 不可用，降级无锁运行）/ false（锁被他人持有，应退出）
     */
    private function tryAcquireLock()
    {
        try {
            ...
            // Redis/锁存储不可用：降级为无锁运行（靠 supervisor numprocs=1 保证单实例）
            Log::warning('[IngressProbe] lock store unavailable, running without lock', ['msg' => $e->getMessage()]);
            return null;
        }
    }
    ...
    /**
     * 主循环。锁在 handle() 已获取并持有整个生命周期，循环内不再触碰锁，
     * 仅刷新心跳 key（owner+timestamp）表达「我还活着」，TTL 略大于一轮耗时。
     */
     ...
     * 多实例并发探活无害（MySQL upsert 幂等）且 supervisor 保证单实例，
     * 故此窗口可接受；续租失败不致命，返回原锁继续。
     */
